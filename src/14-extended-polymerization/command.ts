...
command.action((options: CommandOptionsDF) => {
  try {
    ...
    const part1Polymer = ExtendedPolymerization.extendPolymer(initPolymer, rules, 10);
    const part1FreqMap = ExtendedPolymerization.getFreqMap(part1Polymer);
    const part1Res = freqMaxMinDiff(part1FreqMap);
    console.log(`Part I result: ${part1Res}`);

    const part2Polymer = ExtendedPolymerization.extendPolymer(initPolymer, rules, 40);
    const part2FreqMap = ExtendedPolymerization.getFreqMap(part2Polymer);
