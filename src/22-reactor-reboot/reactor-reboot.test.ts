...
const TEST_CASE3 = {
  ...
  expectedCountOn: 5266495714692187,
};
...
describe("Day 22 - Reactor Reboot", () => {
  ...
  describe("Part II", () => {
    it("ReactorReboot.setCombination() works", () => {
      expect(ReactorReboot.combination(0, 2, 1)).to.eql([[0], [1], [2]]);

      // prettier-ignore
      expect(ReactorReboot.combination(0, 2, 2)).to.eql([[0, 1], [0, 2], [1, 2]]);

      // prettier-ignore
      expect(
        ReactorReboot.combination(0, 4, 3)
      ).to.eql([
        [0, 1, 2], [0, 1, 3], [0, 1, 4],
        [0, 2, 3], [0, 2, 4], [0, 3, 4],
        [1, 2, 3], [1, 2, 4], [1, 3, 4],
    ...
    it("ReactorReboot.intersectionVol() works", () => {
      const cuboid1 = {
        on: true,
        x: { min: 0, max: 2 },
        y: { min: 0, max: 2 },
      ...
      expect(ReactorReboot.intersectionVol([cuboid1, cuboid2])).to.eq(12);
      expect(ReactorReboot.intersectionVol([cuboid1, cuboid2, cuboid3])).to.eq(8);
    ...
    it("ReactorReboot.fullReboot() works on small example", () => {
      const input = [
        ...
        "off x=1..3,y=1..3,z=0..2",
        "on x=2..4,y=-1..1,z=0..2",
      ];

      expect(ReactorReboot.fullReboot(input.slice(0, 2))).to.eq(15);
      expect(ReactorReboot.fullReboot(input)).to.eq(39);
    });

    it("ReactorReboot.fullReboot() works on given example", () => {
      // TEST_CASE3;
      const { input, expectedCountOn } = TEST_CASE3;
      expect(ReactorReboot.fullReboot(input)).to.eq(expectedCountOn);
    });
  });
});
