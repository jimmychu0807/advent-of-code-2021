...
class Amphipod {
  static constructGameState(config: InitConfig): GameState {
    ...
    const pcs = config.roomContent.reduce((acc: PieceState[], room, roomIdx) =>
  ...
  static solve(initConfig: InitConfig) {
    let gameState: GameState = this.constructGameState(initConfig);
    let startPath: Path = { moves: [], totalCost: 0 };
  ...
  static recSolve(initConfig: InitConfig, gameState: GameState, path: Path): Path[] {
    ...
    for (let pcIdx = 0; pcIdx < pcs.length; pcIdx++) {
      ...
      for (let movIdx = 0; movIdx < validMoves.length; movIdx++) {
        const { type, at } = validMoves[movIdx];

        // Updating the state: path
        const step = { pc: pcState.pc, loc: { type, at }};
        path.moves.push(step);
        path.totalCost += this.getMoveCost(pcState, validMoves[movIdx], initConfig.costs);
  ...
  static getValidMoves(pcIdx: number, initConfig: InitConfig, gameState: GameState): Loc[] {
    ...
    if (type === 'c') {
      ...
      let [small, large] = (at as number) > destRoomLoc
        ? [destRoomLoc, at as number]
        : [at as number, destRoomLoc];
    ...
    for (let cIdx = curCorrLoc - 1; cIdx >= 0; cIdx--) {
      ...
      if (!initConfig.roomLoc.includes(cIdx)) { // we don't allow stopping in front of a room
